...
LOGS_URL="https://${logs_user}:${logs_token}@${logs_hostname}/loki/api/v1/push" docker-compose -f "${SCRIPT_DIR}"/docker-compose.yml up "$@"

for dir in "${SCRIPT_DIR}/.data-compactor" \
    "${SCRIPT_DIR}/.data-ingester-1" \
    "${SCRIPT_DIR}/.data-ingester-2" \
    "${SCRIPT_DIR}/.data-minio" \
    "${SCRIPT_DIR}/.data-querier"; do
  sudo rm -Rf "$dir"
done
